...
postfix_expression: primary_expression
                  ...
                  | postfix_expression OPEN_PARENTHESIS argument_expression_list_opt CLOSE_PARENTHESIS
                  {
                        // Function call
                        $$ = new Array();
                        // Get the type of the return variable by peeping into the symbol table of the function
                        Symbol * rt_type = STS.global()->lookup($1->array->name)->nested_table->find("return");
                        if (rt_type != nullptr) {
                              // Non - Void type
                              $$->array = ST->gentemp(rt_type->type);
                        }
                        else {
                              // Void type
...
function_definition: declaration_specifiers declarator declaration_list_opt changetable compound_statement
                   { 
                        ...
                        // Update the type of the return variable
                        Symbol *rt_val = ST->find("return");
                        if (rt_val != nullptr) {
                              rt_val->update($2->type);     // Update the type of the return variable for non-void function
                        }
                        // cout << $2->type->name << "\n";
